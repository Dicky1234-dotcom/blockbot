...
async function handleMessage(ctx) {
  ...
  try { await db.ensureUser(userId, ctx.from.username); } catch (dbErr) {
    console.error('ensureUser DB error:', dbErr.message, dbErr);
    return ctx.reply('❌ Database error: ' + dbErr.message + '\n\nCheck your SUPABASE_URL and SUPABASE_ANON_KEY in .env, and make sure your schema.sql has been applied.');
  }
  ...
  try {
    ...
    await ctx.reply('❌ Handler error: ' + (err.message || String(err)) + '\n\nCheck server logs. Type /help for commands.');
  }
}
